...
public class AjaxSeotdaController {
	...
	public String connectAjax(Model model, CardDeck carddeck, Card card) {
		...
		// 승부 판별용 점수만 따로 계산
		int firstHandedNum = carddeck.getCards().get(0).getNum();
		int secondHandedNum = carddeck.getCards().get(2).getNum();
		// 1번 플레이어 총 합 점수
...
//		model.addAttribute("inputPageCardList", inputPageCardList);
		...
		model.addAttribute("totalScore", firstPlayerTotalScore);
...
//		CardDeck cd = new CardDeck();
		...
		for (int i = 1; i < first.length()-1; i++) {
			if (!firstHanded.contains(String.valueOf(first.charAt(i)))) {
				firstHanded+=String.valueOf(first.charAt(i));
			}
		}
